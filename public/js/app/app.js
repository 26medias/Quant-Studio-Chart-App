...
.directive('appCharts', ['$timeout', function ($timeout) {
	var component = function($scope, element, attrs, ctlr, transcludeFn) {
		...
		$scope.tabs	= {
			...
			is:		function(id) {
				return $scope.tabs.selected && $scope.tabs.selected.id	== id;
			},
		...
		$scope.chart	= {
			...
			init:	function() {
				...
				// Was there some charting ont he project?
				if (!$scope.app.charting && $scope.project.charting) {
					$scope.app.charting = $scope.project.charting;	// Copy
					// Save
			...
			removePlot:	function(plot) {
				$scope.app.charting.plots = _.filter($scope.app.charting.plots, function(item) {
					return item.id!=plot.id;
				});
				
				$scope.app.charting.plots = _.map($scope.app.charting.plots, function(item, n) {
					item.order	= n;
					return item;
				});
			...
			refresh:	function() {
				...
				console.log("Chart: refresh", $scope.app);
				
				if (!$scope.app || !$scope.datasets || _.keys($scope.datasets).length==0) {
					return false;
				}
				...
				_.each($scope.app.charting.plots, function(plotData) {
					// Events
					if (plotData.events) {
				...
				if ($scope.app.charting.scrollerRange) {
					chartInstance.selectRange($scope.app.charting.scrollerRange.from,$scope.app.charting.scrollerRange.to);
				}
				
				// Save the scroller zoom when it changes
				chartInstance.listen("selectedrangechangefinish", function(e){
					$scope.app.charting.scrollerRange = {
				...
				boundItv = setInterval(function() {
					_.each(plotRefs, function(plot,k) {
						...
						var plotData = _.find($scope.app.charting.plots, function(item) {
							return item.id == k;
						});
						if (plotData) {
			...
			addPlot:	function(options) {
				...
				var plot = {
					...
					charts: [],
					events: [],
					order:	$scope.app.charting.plots.length
				};
			...
			addChart:	function(options) {
				options = _.extend({}, options, {});
				var plot = _.find($scope.app.charting.plots, function(item) {
